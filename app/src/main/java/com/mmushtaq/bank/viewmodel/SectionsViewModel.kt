...
class SectionsViewModel : ViewModel() {
    private var sectionLiveList: MutableLiveData<ArrayList<Question>> = MutableLiveData<ArrayList<Question>>()
    ...
    fun saveCase(cases : ArrayList<Case>)
    {
        val userService = NetworkClient.createService(UserService::class.java)
        val call = userService.updateCases(SharedPreferences.getSharedPreferences("access-token", BaseApplication.getContext()),
                SharedPreferences.getSharedPreferences("client", BaseApplication.getContext()),
                SharedPreferences.getSharedPreferences("uid", BaseApplication.getContext()), "*/*", convertFields(cases))
    ...
    {
        val userService = NetworkClient.createService(UserService::class.java)
        val call = userService.getCases(SharedPreferences.getSharedPreferences("access-token", BaseApplication.getContext()),
                SharedPreferences.getSharedPreferences("client", BaseApplication.getContext()),
            ...
            cases.forEach {
                    ...
                    if (agent_ques.type == "agent_info") {
                        agent_ques.questions.forEach { question ->
                            ...
                            if (question.keyboard_type.toString() == "current_date") {
                                map["cases[$i][case_question_answers_attributes][$j][question_id]"] = question.id.toString()
                                map["cases[$i][case_question_answers_attributes][$j][answer_description]"] = question.given_answer!!
                        ...
                        agent_ques.questions?.forEach { questions ->
                            ...
                            map["cases[$i][case_question_answers_attributes][$j][question_id]"] = questions.id.toString()
                            if (null != questions.answerId) {
                                map["cases[$i][case_question_answers_attributes][$j][answer_id]"] = questions.answerId
                            } else {
                                map["cases[$i][case_question_answers_attributes][$j][answer_id]"] = ""
                            }
                            if (null != questions.remarks) {
                                map["cases[$i][case_question_answers_attributes][$j][remarks]"] = questions.remarks.toString()
                            }
                            if (null != questions.selectedAnswer) {
                                map["cases[$i][case_question_answers_attributes][$j][answer_description]"] = questions.selectedAnswer.toString()
                            }
                ...
                it.documents_residence_attributes?.forEach { residence_docs ->
                    if (null != residence_docs.documentUrl) {
                        map["cases[$i][documents_attributes][$x][id]"] = residence_docs.id.toString()

                        map["cases[$i][documents_attributes][$x][doc]"] = residence_docs.documentUrl.toString()

                        if (null != residence_docs.longitude) {
                            map["cases[$i][documents_attributes][$x][longitude]"] = residence_docs.longitude.toString()
                        }
                        if (null != residence_docs.latitude) {
                            map["cases[$i][documents_attributes][$x][latitude]"] = residence_docs.latitude.toString()
                        }
                    }
                    x++
                ...
                if (null != it.documents_business_attributes) {
                    it.documents_business_attributes?.forEach { business_docs ->
                        if (null != business_docs.documentUrl) {
                            map["cases[$i][documents_attributes][$x][id]"] = business_docs.id.toString()

                            map["cases[$i][documents_attributes][$x][doc]"] = business_docs.documentUrl.toString()

                            if (null != business_docs.longitude) {
                                map["cases[$i][documents_attributes][$x][longitude]"] = business_docs.longitude.toString()
                            }
                            if (null != business_docs.latitude) {
                                map["cases[$i][documents_attributes][$x][latitude]"] = business_docs.latitude.toString()
                            }
                        }
                        x++
